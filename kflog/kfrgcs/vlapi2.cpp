...
int32 VLA_XFR::commandbaud = 9600;
...
int32 VLA_XFR::readlog(lpb puffer, int32 maxlen) {
  ...
  while(!ende) {
    ...
    while( !test_user_break() && serial_in(&c) );
...
// >0 : logger present
//
VLA_ERROR VLAPI::open(boolean connectit, int timeout, boolean quiet, int32 sbaudrate) {
...
VLA_ERROR VLAPI::read_db_and_declaration() {
	...
	if (dbb1.header[0].dsfirst != 0xffff) {
		database.nwpts = 1 +
			(dbb1.header[0].dslast - dbb1.header[0].dsfirst) 
		...
		for (int i=0; i<database.nwpts; i++)
			database.wpts[i].get(
				dbb1.block + dbb1.header[0].dsfirst + 
	...
	if (dbb1.header[3].dsfirst != 0xffff) {
		database.nroutes = 1 +
			(dbb1.header[3].dslast - dbb1.header[3].dsfirst) 
		...
		for (int i=0; i<database.nroutes; i++) 
			database.routes[i].get(
				dbb1.block + dbb1.header[3].dsfirst + 
	...
	if (dbb1.header[1].dsfirst != 0xffff) {
		database.npilots = 1 +
			(dbb1.header[1].dslast - dbb1.header[1].dsfirst) 
		...
		for (int i=0; i<database.npilots; i++) 
			database.pilots[i].get(
				dbb1.block + dbb1.header[1].dsfirst + 
...
void VLAPI_DATA::WPT::get(lpb p) {
 int32 ll;
  ...
  lat = (float) (ll / 60000.0);
  if (p[7] & 0x80)
    lat = -lat;
  ll = (int32) (65536.0 * p[10] + 256 * p[11] + p[12]);
  lon = (float) (ll / 60000.0);
  if (p[6] & 0x80)
    lon = -lon;
}
...
void VLAPI_DATA::ROUTE::put(lpb p) {
  strupr(name);
  memcpy(p,name,14);
  for(int i=strlen((char *)p); i<14; i++)
    p[i] = ' ';
...
void VLAPI_DATA::DECLARATION::put(DBB *dbb) {
  ...
  dbb->add_fdf(
		FLDGTY,
		strlen(flightinfo.glidertype)+1,
		flightinfo.glidertype
	);
  
	dbb->add_fdf(
		FLDGID,

		strlen(flightinfo.gliderid)+1,
		flightinfo.gliderid
	);
  
	dbb->add_fdf(
		FLDCCL,
		strlen(flightinfo.competitionclass)+1,
		flightinfo.competitionclass
	);
  
	dbb->add_fdf(
		FLDCID,
		strlen(flightinfo.competitionid)+1,

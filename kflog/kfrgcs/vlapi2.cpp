...
int16 VLA_XFR::sendcommand(byte cmd, byte param1, byte param2) {
  int16       	i;
  byte	        c;
  const int16    d = 2;  //Verz�gerungszeit 2ms
  byte 		cmdarray[8];
  word   crc16 = 0;
  int32	 	t1;
  int16 		timeout = 4;
  ...
  // Kommandointerpreter im VL zur�cksetzen
  for(i=0; i<6; i++) {
    serial_out(CAN);
    wait_ms(d);
  }
  ...
  serial_out(crc16%256);
  wait_ms(d);
  // Kommandobest�tigung abwarten, aber h�chstens timeout Sekunden
  t1 = get_timer_s();
  ...
  // Fehlercode als R�ckgabewert der Funktion benutzen
  return c;
  // R�ckgabewert:     -1 : timeout
  //		 0..255 : Best�tigungscode vom Logger
}
...
//     puffer[i] = 0xff;
  ...
  while(!ende) {
    ...
    // dabei ist Benutzerabbruch jederzeit m�glich
    if (test_user_break()) {
      if (clear_user_break() == 1) {
        ende = -1;
        wait_ms(10);
...
VLA_ERROR VLA_XFR::dbbput(lpb dbbbuffer, int32 dbbsize) {
  ...
  serial_empty_io_buffers();
  sendcommand(cmd_PDB,0,0); // mu� noch mit Timeout versehen werden
  // auf L�schende warten
...
VLA_ERROR VLA_XFR::connect(int32 waittime, int quietmode ) { 
  ...
  do { // Solange R's aussenden, bis ein L zur�ckkommt
    serial_out('R');
    wait_ms(30);

  ...
  if(updateAll)
    {
      ...
      htmlText = (QString) "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0>\
                            <TR><TD><B>Kreisflug</B></TR></TR> \
                            <TR><TD><B>Zeit</B></TD> \
                                <TD><B>Vario</B></TD> \
                                <TD><B>Höhengewinn</B></TD> \
                                <TD><B>Höhenverlust</B></TD></TR>";
      ...
      htmlText += (QString) "<TR></TR><TR><TD><B>Strecke</B></TR> \
                                      <TR><TD><B>Zeit</B></TD> \
                                      <TD><B>ges. Strecke</B></TD> \
                                      <TD><B>Höhengewinn</B></TD> \
                                      <TD><B>Höhenverlust</B></TD> \
                                      <TD><B>Geschwindigkeit</B></TD> \
                                      <TD><B>Gleitzahl</B></TD></TR>";
      ...
      htmlText += (QString) "<TR></TR><TR><TD><B>Gesamt:</B></TD></TR>" + \
...
{
warning("EvaluationDialog::updateListBox()");
  combo_flight->clear();
  ...
  for(unsigned int n = 0; n < flightList->count(); n++)
  {
      ...
      warning(flightList->at(n)->getFileName());
  }
...
{
warning("EvaluationDialog::slotShowFlightData(%d)", n);
  this->setCaption(i18n("Flugauswertung: ") + flightList->at(n)->getPilot()
...
void EvaluationDialog::slotScale(unsigned int g)
{
  ...
  flightList(fList)
{
  ...
  setCaption(i18n("Flugauswertung:"));
  setMinimumWidth(500);
...
//  check_vario->setMinimumWidth(check_vario->sizeHint().width());
  ...
  spin_vario = new QSpinBox(0,999,1,this);
  spin_baro = new QSpinBox(0,999,1,this);
  spin_speed = new QSpinBox(0,999,1,this);

  QLabel* scale_label = new QLabel("Zeitskalierung:", oben);
  scale_label->setAlignment(AlignRight);
  spin_scale = new QSpinBox(1,60,1,oben);
  spin_scale->setMaximumWidth(60);


  // Schließen
  QPushButton* close = new QPushButton("Schließen",this);
  ...
  obenlayout->addWidget(spin_scale);
...
//  tabLayout->addRowSpacing(13,0);
  ...
  this->connect(spin_vario, SIGNAL(valueChanged(int)),
        SLOT(slotVarioGlatt(int)));
  this->connect(spin_speed, SIGNAL(valueChanged(int)),
        SLOT(slotSpeedGlatt(int)));
  this->connect(spin_baro, SIGNAL(valueChanged(int)),
        SLOT(slotBaroGlatt(int)));
  this->connect(close, SIGNAL(clicked()), SLOT(reject()));

  this->connect(spin_scale, SIGNAL(valueChanged(int)),
        SLOT(slotScale(unsigned int)));

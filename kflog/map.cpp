  ...
  preSnapPoint(-999, -999)
{
  ...
  setWhatsThis( tr(
     "<html><b>The map help</b><br><br>"
     "Map zooming in or out can be done:"
     "<ul>"
     "<li>With the slider of the <i>Map Control</i> window."
     "<li>With the \"+\" or \"-\" button of the <i>Toolbar</i>."
     "<li>With the shortcut keys \"&lt;Ctrl&gt;&lt;+&gt;\" and \"&lt;Ctrl&gt;&lt;-&gt;\"."
     "<li>With the mouse wheel."
     "</ul>"
     "Map moving can be done:"
     "<ul>"
     "<li>With the buttons of the <i>Map Control</i> window."
     "<li>With the <i>cursor</i> keys and the keys on the <i>NumPad</i> if <i>NumLock</i> is switched on. "
     "The key 5 is the center point and the other number keys around are moving the map."
     "<li>With pressing the <i>left</i> mouse button down and mouse moving."
     "</ul>"
     "Supported mouse actions at the map:"
     "<ul>"
     ...
     "<li>Pressing the <i>Control</i> key and the <i>left</i> mouse button "
     "and hold both down during drag will draw a frame at the map. Release the mouse "
     "button to zoom the map into the frame."
     "<li>Turning the mouse <i>wheel</i> will zoom into or out the map."
     "</ul>"
     "<P>Open the menu page <i>Settings-&gt;KFLog Setup-&gt;Map Elements</i> to "
     "configure, which elements should be displayed at which scale at the map.</P>"
     "</html>"
      ) );
  ...
  if(baseFlight && baseFlight->getTypeID() == BaseMapElement::Flight)
    {
      ...
      for(int loop = 0; loop < wpList.count(); loop++)
        {
          QPoint sitePos = _globalMapMatrix->map(wpList.at(loop)->projP);
...
bool Map::__getTaskWaypoint( const QPoint& current,
                             Waypoint *wp,
...
bool Map::findMapPoint( int delta, const QPoint& mapPosition, Waypoint *w )
{
  // compose search list
  QList<int> searchList;
  searchList << MapContents::GliderfieldList
